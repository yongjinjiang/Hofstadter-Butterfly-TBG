...
d_0=0.335;    #nm:  all interplane distance are in unit of (delta_0)
...
def make_system_step2(p,k1,k2,syst,syst0,nns,SC1,SC2,q):
    ...
    def amp(ri,rj):
      ...
      #print('hop_t=',hop_t)
      #monolayer: the usual nearest hopping model:#
      #hop_t=0;
      if abs(zi-zj)<10**-8:
         hop_t=0;
         if abs(dis-delta_a)<=10**-6:
            hop_t=t;
      return -hop_t
    ...
    def phase(ri,rj):
       ...
       #ss2_i=s2x*xR+s2y*yR
       ss1_i=(sqrt(3)*m1*xR+2*sqrt(3)*m2*xR - 3*m1*yR)/(3*(m1**2 + m1* m2 + m2**2))  +0.33*eta;
       ss2_i=(sqrt(3)*m1*xR - sqrt(3)* m2*xR + 3* m1* yR +3* m2* yR)/(3* (m1**2 + m1*m2 + m2**2));
       xR,yR,zj=rj;
       ss1_j=(sqrt(3)*m1*xR+2*sqrt(3)*m2*xR - 3*m1*yR)/(3*(m1**2 + m1* m2 + m2**2)) +0.3003*eta ;
       ...
       if abs(np.floor(ss1_i)-np.floor(ss1_j))==1:
          ...
          phase2 = -(ss*ss2_i + (1 - ss)*ss2_j)*np.sign(ss1_i-ss1_j);
       
       phase1= -(ss2_i-ss2_j)*(np.floor(ss1_j)*(mn1-ss1_j)/(ss1_i-ss1_j) +np.floor(ss1_i)*(ss1_i-mn1)/(ss1_i-ss1_j))  + (ss1_i+ss1_j)/2*(ss2_i-ss2_j);
...
#          
          ...
          #the periocity of phase factor:
            ...
            print(phase(ri,rj))
            print(phase(ri+SC1,rj+SC1))
            print(phase(ri+6*tinyarray.array(L2),rj+6*tinyarray.array(L2)))
          ...
          print(phase(ri+q*tinyarray.array(L2),rj+q*tinyarray.array(L2)))
          print('q=',q,'p=',p)
          print('total phase=',(phase(r[0,:],r[5,:])*phase(r[1,:],r[0,:])*phase(r[2,:],r[1,:])*phase(r[3,:],r[2,:])*phase(r[4,:],r[3,:])*phase(r[5,:],r[4,:]))**q)
          return '1111here'
...
def main():
    ...
    Bfields=range(0,q*(m1*m2+m1**2+m2**2),10);
    Num_p=len(Bfields);
    ...
    #for nk in range(len(k2_list)):
        ...
        #print(syst[a(0,0),b(0,0)])
        syst=syst.finalized()
#        print('eeeeeeeeeee')
#        print(syst.sites[0].tag,syst.sites[1].tag,syst.sites[2].tag,syst.sites[3].tag)
#        print(syst.sites[0].family,syst.sites[1].family,syst.sites[2].family,syst.sites[3].family)
#        print(syst.sites[0].pos,syst.sites[1].pos,syst.sites[2].pos,syst.sites[3].pos)
#
        ham_mat= syst.hamiltonian_submatrix(args=[], sparse=False)
#        print(ham_mat)
#        print('----------------------')
#        print(syst.hamiltonian_submatrix(args=[],to_sites=(0,2),from_sites=(0,2)))
#        print('----------------------')
#        print(syst.hamiltonian_submatrix(args=[],to_sites=(1,3),from_sites=(1,3)))
#        sys.exit()
#
...
#        print('nk_kpm=',nk)
    ...
    filename='Butterfly_Bilayer_with_B_q={0}-m1={1}-m2={2}AB.dat'.format(q,m1,m2);
    #res = [x, y, z, ....]
...
#       writer.writerows(energies_kpm)
    ...
    plt.figure()
    plt.plot(Bfields, energies,'-');plt.xlabel("magnetic field [arbitrary units]")
   # plt.plot(k1_list, energies,'*')
    #plt.plot(range(len(k2_list)), np.array(energies),'-');plt.xlabel("from  K to Gamma to M to K")
